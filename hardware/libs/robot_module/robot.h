...
RoboticArm::RoboticArm() {
    servos[0] = new Servo(0, 90, 30, 210); // Gripper(aj End Effector) funguje rovnako ako rotation serva, len namiesto uhlov pracuje so sirkov chapadla, v tomto pripade 30 mm su najsirsie, 0 su zatvorene
    servos[1] = new Servo(0, 232, 125, 110);
    servos[2] = new Servo(0, 60, 151, 210);
    servos[3] = new Servo(0, 3300, 90, 2190);
    servos[4] = new Servo(-180, 3450, 13, 670);
...
void sendPacket_old(int port, string packet) {
    const char *toSend = packet.c_str();
    bool isServoPacket = (packet[1] == 's');
    ...
    for (int i = 0; i < strlen(toSend); i++) {
        //for(int i = 0; i < strlen(packet.c_str()); i++){
        //  int written = write(port, &(packet[i]), 1);
        int written = write(port, toSend + i, 1);

        if (isServoPacket) {
            usleep(10000); // aby to platilo iba pri servomotoroch, motory ATtiny to zvlada aj bez toho
        }
